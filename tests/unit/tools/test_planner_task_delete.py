...
"""Unit tests for planner_task_delete."""
...
def _patch_get_token(monkeypatch: pytest.MonkeyPatch, token: str = "AT") -> None:
    monkeypatch.setattr(
        "microsoft_tasks_mcp.tools.planner_task_delete.get_token",
        lambda profile: token,
    )


def _seed(tmp_path: Path, etag: str | None = 'W/"e"') -> TaskRegistry:
...
def test_deletes_and_removes_from_registry(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    delete_planner_task("T1", registry=reg)
    assert reg.get("T1") is None


@respx.mock
def test_passes_if_match(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _patch_get_token(monkeypatch)
    reg = _seed(tmp_path, etag='W/"v1"')
    route = respx.delete(URL).respond(204)
    delete_planner_task("T1", registry=reg)
    assert route.calls.last.request.headers["If-Match"] == 'W/"v1"'


@respx.mock
def test_404_treated_as_success(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _patch_get_token(monkeypatch)
    reg = _seed(tmp_path)
    respx.delete(URL).respond(404, json={"error": "ItemNotFound"})
    delete_planner_task("T1", registry=reg)
    assert reg.get("T1") is None
...
def test_rejects_empty_task_id(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="non-empty task_id"):
        delete_planner_task("", registry=TaskRegistry("default", base_dir=tmp_path))
...
def test_propagates_500(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    _patch_get_token(monkeypatch)
    reg = _seed(tmp_path)
    respx.delete(URL).respond(500, json={"error": "internal"})
